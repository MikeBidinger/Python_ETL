...
if __name__ == "__main__":
    ...
    # Parse XML data
    xml_data = E_XML(CONFIG["extract"]["xml"]).parse()

    # Parse JSON data
    json_data = E_JSON(CONFIG["extract"]["json"]).parse()
    ...
    # Join data (left join)
    join = T_Join(CONFIG["transform"]["join"]).join(del_data, lookup)
    ...
    # Write joined data as XML
    file_path = L_XML(CONFIG["load"]["xml"]).load(join.copy())
    print(file_path)

    # Write joined data as JSON
    file_path = L_JSON(CONFIG["load"]["json"]).load(join.copy())
    print(file_path)
