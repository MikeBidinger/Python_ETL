...
def write_xml(data, file_path, append=False):
    if append:
        mode = "a"
    else:
        mode = "w"
    my_item_func = lambda x: "row"
    xml = dicttoxml(data, attr_type=False, item_func=my_item_func)
    dom = parseString(xml)
    with open(file_path, mode) as f:
    ...
    def load(self, data):
        ...
        for row in data[1:]:
            ...
            if columns == []:
                ...
                for idy, col in enumerate(columns):
                    xml[-1][data[0][columns[idy]]] = row[col]
    ...
    def load(self, data):
        ...
        for row in data[1:]:
            ...
            if columns == []:
                ...
                for idy, col in enumerate(columns):
                    json[-1][data[0][columns[idy]]] = row[col]
